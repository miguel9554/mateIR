...
// Top-level enum test.
// Ports use logic vectors so gen_tb.py can parse them unchanged.
// Internally everything is cast to / driven by enum types.
// Tests:
//   - enum inputs (cast at boundary)
//   - enum outputs (assigned from enum signals)
//   - enum flops (state, color)
//   - enum as FSM next-state case selector
//   - enum comparisons in comb logic
