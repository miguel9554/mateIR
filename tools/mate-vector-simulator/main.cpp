...
void printUsage(const char* progName) {
    std::cerr << "Usage: " << progName << " [options] <source1.v> [source2.v ...]\n"
              ...
              << "  --simulate                Accepted for compatibility; simulation is the only mode\n"
              ...
              << "                            Ignore stored flop declaration initial values;\n"
              << "                            use the --flops-initial policy instead\n"
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "--simulate") == 0) {
            ...
            std::cerr << "ERROR: " << argv[i]
                      << " is not supported by mate-vector-simulator\n";
            printUsage(argv[0]);
            return 1;
    ...
    try {
        ...
        if (topModule.empty()) throw CompilerError("mate-vector-simulator requires --top <module>");
        if (inputsDir.empty()) throw CompilerError("mate-vector-simulator requires --inputs-dir <dir>");
        if (outputDir.empty()) throw CompilerError("mate-vector-simulator requires --output-dir <dir>");
        ...
        frontendOptions.top_module = topModule;
        ...
        std::cout << "========================================\n";
        std::cout << "mate-vector-simulator\n";
        std::cout << "========================================\n";
        std::cout << "Frontend: " << frontendName << "\n";
        std::cout << "Top: " << topModule << "\n";

        auto frontend = makeFrontend(frontendName);
        auto runtimeModel = compileRtlRuntimeModel(*frontend, frontendOptions);
        if (dumpPasses) runtimeModel.ir().top.dumpDfgFiles();
